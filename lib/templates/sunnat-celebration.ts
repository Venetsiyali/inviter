...
export function sunnatCelebration(data: TemplateData): string {
    return `<!DOCTYPE html>
<html lang="uz"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>${data.brideGroom} — Sunnat to'yi</title>
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:'Segoe UI',sans-serif;background:${data.primaryColor};color:#fff;min-height:100vh}
.c{max-width:480px;margin:0 auto;padding:40px 24px;text-align:center}
.icon{font-size:56px;margin-bottom:16px}
.stars{font-size:24px;letter-spacing:8px;margin-bottom:16px;opacity:.5}
.label{font-size:11px;letter-spacing:4px;text-transform:uppercase;color:${data.secondaryColor};opacity:.7;margin-bottom:16px}
.names{font-size:30px;font-weight:700;margin-bottom:8px}
.sub{font-size:15px;opacity:.6;margin-bottom:32px}
...
<div class="sub">sunnat to'yiga taklif etamiz</div>
